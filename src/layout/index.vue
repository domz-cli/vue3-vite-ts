<template>
    <section class="ayout-header-aside-group">
        ...
        <div class="ayout-header-aside-content">
            <!-- 顶栏 -->
            <header class="domz-theme-header" :style="{ opacity: searchActive ? 0.5 : 1 }" flex-box="0" flex>
                ...
                <!-- 顶栏右侧 -->
                <section class="domz-header-right flex-align-center" flex-box="0">
                    <menuHeaderSetting></menuHeaderSetting>
                </section>
            </header>
            <!-- 下面 主体 -->
            <div class="domz-theme-container flex" flex-box="1" flex>
                <!-- 主体 侧边栏 -->
                <menuNav></menuNav>
                <!-- 主体 -->
                <transition name="fade-scale">
                    <div class="domz-theme-container-main" :style="{ width: `calc(100vw - ${asideWidth})` }" :class="{ 'domz-theme-container-noside': layoutPageType }" flex-box="1" flex>
                        <!-- 搜索 -->
                        <transition name="fade-scale">
                            <div v-if="searchActive" class="hsd-theme-container-main-layer" flex>
                                <panel-search ref="panelSearchRef" />
                            </div>
                        </transition>
                        <!-- 内容 -->
                        <transition name="fade-scale">
                            <!-- tab -->
                            <section v-if="!searchActive" class="domz-theme-container-main-layer" flex="dir:top">
                                <!-- <div v-if="!layoutPageType" class="domz-theme-container-main-header" flex-box="0">
                                    <menuTags />
                                </div> -->
                                <!-- 页面 -->
                                <main class="domz-theme-container-main-body" :class="{ 'no-margin': layoutPageType }">
...
import panelSearch from '@/layout/components/panel-search.vue';
...
const searchActive = computed(() => {
    return globalSettingStore.searchOn;
});
...
const { transitionActive, keepAlive, processEnv, pmenu, menu, active, nextMenu } = toRefs(reactiveObj);
