...
import { filterEnum, formatValue, handleRowAccordingToProp } from '@/utils/tool';
...
const renderLoop = (item: ColumnProps) => {
    return (
        <>
            {item.isShow && (
                <el-table-column onClick={() => console.log(1)} {...item} align={item.align ?? 'center'} showOverflowTooltip={item.showOverflowTooltip ?? item.prop !== 'operation'}>
                    {{
                        default: (scope: any) => {
