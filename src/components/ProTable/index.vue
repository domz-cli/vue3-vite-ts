<template>
    <section class="table-box">
        <!-- 查询表单 card -->
        <SearchForm :cardStyle="cardStyle" v-show="isShowSearch" :search="search" :reset="reset" :search-param="searchParam" :columns="searchColumns" :search-col="searchCol" />

        <!-- 表格内容 card -->
        <div class="card table-main" :class="{ 'card-style': cardStyle }">
            <!-- 表格头部 操作按钮 -->
            <div class="table-header flex-space-center">
            ...
            <el-table ref="tableRef" v-bind="$attrs" :data="tableData" :border="border" :row-key="getRowKeys" @selection-change="selectionChange">
                ...
                <template v-else>
                    <template v-for="item in tableColumns" :key="item">
                        <!-- selection || index -->
                        <el-table-column v-if="item.type == 'selection' || item.type == 'index'" v-bind="item" :align="item.align ?? 'center'" :reserve-selection="item.type == 'selection'">
                        </el-table-column>
                ...
                <template #empty>
                    <div class="table-empty">
                        <slot name="empty">
                            <img src="@/assets/img/empty-page.png" alt="notData" />
                            <div>暂无数据</div>
                        </slot>
                    </div>
                </template>
            ...
            <slot name="pagination">
                <section class="flex-space-center">
...
import { filterEnum, formatValue, handleProp, handleRowAccordingToProp } from '@/utils/tool';
...
interface ProTableProps extends Partial<Omit<TableProps<any>, 'data'>> {
    cardStyle: false; // 卡片模式：搜索栏与table，之间没有间距与border
    isShowSearch: true; // 是否显示搜索模块
...
const setEnumMap = async (col: ColumnProps) => {
    ...
    enumMap.value.set(col.prop!, data instanceof Array ? data : data.rows || []);
};
